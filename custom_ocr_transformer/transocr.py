...
class Transformer(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.encoder = ResNet(num_in=3, block=BasicBlock, layers=[3,4,6,3], args=args)
    ...
    def forward(self, image, text_length, text_input, radical_length=None, radical_input=None, conv_feature=None, test=False, att_map=None):
        ...
        if test == False and self.args.radical:
            ...
            postion_embedding_radical = self.pe_radical(torch.zeros(embedding_radical.shape))
...
if __name__ == '__main__':
    net = ResNet(num_in=3, block=BasicBlock, layers=[3, 4, 6, 3])
    image = torch.Tensor(8, 3, 64, 64)
    result = net(image)
    print(result.shape)
    pass